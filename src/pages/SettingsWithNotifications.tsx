...
function SoftwareRegistryManagement() {
  ...
  const deletePolicyItem = async (item: PolicyItem) => {
    const itemId = Number(item.PolicyItemID || 0);
    if (!itemId) {
      setMessage({ type: "error", text: "Invalid software item ID. Please reload and try again." });
      return;
    }

    if (!window.confirm(`Remove ${item.SoftwareName} from this registry?`)) return;

    setDeletingItemId(itemId);
    try {
      await api.delete(`${API_ROOT}/items/${itemId}`);
      setPolicyItems((current) => current.filter((row) => Number(row.PolicyItemID) !== itemId));
      if (activePolicyId) {
        await loadPolicyItems(activePolicyId);
      }
      await loadPolicies();
      setSelectedSoftware((current) => (current && getSoftwareKey(current) === getSoftwareKey(item) ? null : current));
      setMessage({ type: "success", text: "Software item removed from registry." });
    } catch (error) {
      setMessage({ type: "error", text: pickErrorMessage(error, "Failed to remove software item.") });
    } finally {
  ...
  const deleteRegistryPolicy = async (policy: PolicyRow) => {
    const policyId = Number(policy.PolicyID || 0);

    if (!policyId) {
      setMessage({ type: "error", text: "Invalid software registry ID." });
      return;
    }
    ...
    const removeFromUi = () => {
      setPolicies((current) => current.filter((row) => Number(row.PolicyID) !== policyId));

      if (activePolicyId === policyId) {
        setActivePolicyId(null);
        setPolicyItems([]);
        setSelectedSoftware(null);
      }
    ...
    try {
      await api.delete(`${API_ROOT}/policies/${policyId}`);
      removeFromUi();
      setMessage({ type: "success", text: "Software registry deleted." });
    } catch (error) {
      const errorMessage = pickErrorMessage(error, "Failed to delete software registry.");

      if (/404|not found/i.test(errorMessage)) {
        removeFromUi();
        setMessage({ type: "success", text: "Software registry already deleted / removed from list." });
        return;
      }
                  ...
                  {loading ? <div className="sp-empty">Loading software registry...</div> : policies.length === 0 ? <div className="sp-empty">No software registry found. Click Register Software to create the first entry.</div> : policies.map((policy) => (
                    <div key={policy.PolicyID} className="sp-policy-table-row">
                      ...
                      <span className="sp-policy-table-actions">
                        ...
                        <button className="sp-danger" type="button" title="Delete" aria-label="Delete registry" onClick={() => deleteRegistryPolicy(policy)} disabled={deletingPolicyId === Number(policy.PolicyID)}><Trash2 size={14} /></button>
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="sp-policy-modal-body">
                ...
                {activePolicy && (
                  <section className="sp-section">
                    <div className="sp-section-title"><strong>Registered software item(s)</strong><small>Delete here removes only the software item. Delete on the list removes the whole registry.</small></div>
                    <div className="sp-section-body">
                      <div className="sp-table">
                        <div className="sp-row head sp-registered-row"><span>Software</span><span>Publisher</span><span>Status</span><span>License</span><span></span></div>
                        {policyItems.length === 0 ? <div className="sp-empty">No software item registered under this policy yet.</div> : policyItems.map((item) => {
                          const itemId = Number(item.PolicyItemID || 0);
                          return (
                            <div key={item.PolicyItemID || getSoftwareKey(item)} className="sp-row sp-registered-row">
                              <span><strong>{item.SoftwareName}</strong><small>{item.Version || item.CategoryName || "No version"}</small></span>
                              <span>{item.Publisher || "Unknown"}</span>
                              <span><b className={`sp-badge ${(item.ComplianceStatus || item.Classification) === "Illegal" ? "illegal" : "legal"}`}>{item.ComplianceStatus || item.Classification || "Legal"}</b></span>
                              <span>{item.LicenseCount || 0}</span>
                              <span><button className="sp-danger" type="button" title="Remove software item" aria-label="Remove software item" onClick={() => deletePolicyItem(item)} disabled={deletingItemId === itemId || saving}><Trash2 size={14} /></button></span>
                            </div>
                          );
                        })}
                      </div>
